...
const defaultProgramBlockEditorProps = {
    ...
    programSequence: new ProgramSequence([{block: 'forward'}, {block: 'left45'}, {block: 'forward'}, {block: 'left45'}], 0, 0),
...
describe("Add nodes", () => {
    test("All aria labels for add buttons should be correct when no action is selected.", () => {
        ...
        const { wrapper } = createMountProgramBlockEditor({
            programSequence: new ProgramSequence([{block: 'forward'}, {block: 'right45'}], 0, 0),
            addNodeExpandedMode: true
        });
...
describe("Add program steps", () => {
    test('When the add node at the end of the program is clicked, then the onInsertSelectedActionIntoProgram callback should be called', () => {
        ...
        const { wrapper, mockInsertSelectedActionIntoProgramHandler } = createMountProgramBlockEditor({
            programSequence: new ProgramSequence([{block: 'forward'}, {block: 'forward'}, {block: 'forward'}, {block: 'forward'}, {block: 'forward'}], 0, 0),
            selectedAction: 'left45'
        });
...
describe('Autoscroll to show a step after the active program step', () => {
    ...
    test('When a step after active program block is outside of the container, on the right', () => {
        ...
        wrapper.setProps({
            ...
            programSequence: new ProgramSequence([{block: 'forward'}, {block: 'left45'}, {block: 'forward'}, {block: 'left45'}], 2, 0)
        });
    ...
    test('When active program block is the last program block, autoscroll to the last add node', () => {
        ...
        wrapper.setProps({
            runningState: 'running',
            programSequence: new ProgramSequence([{block: 'forward'}, {block: 'left45'}, {block: 'forward'}, {block: 'left45'}], 3, 0)
        });
...
test('focusCommandBlockAfterUpdate', () => {
    ...
    wrapper.setProps({ programSequence: new ProgramSequence([{block: 'forward'}, {block: 'forward3'}, {block: 'forward2'}], 0, 0) });
...
test('focusAddNodeAfterUpdate', () => {
    ...
    // And the program is updated
    wrapper.setProps({ programSequence: new ProgramSequence([{block: 'forward'}], 0, 0) });
...
test('scrollToAddNodeAfterUpdate', () => {
    ...
    // And the program is updated
    wrapper.setProps({ programSequence: new ProgramSequence([{block: 'forward'}, {block: 'forward'}, {block: 'forward'}, {block: 'forward'}, {block: 'forward'}, {block: 'left45'}], 0, 0) });
