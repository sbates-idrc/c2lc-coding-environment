...
describe('Character position gets updated on character-position div', () => {
    test('When characterState prop is changed', () => {
        ...
        expect(document.getElementById('someAriaLiveRegionId').innerText).toBe('the robot at column A, row 1 facing right');
        ...
        // $FlowFixMe: Flow doesn't know about character-position div
        expect(document.getElementById('someAriaLiveRegionId').innerText).toBe('the jeep at column B, row 1 facing right');
        ...
        // $FlowFixMe: Flow doesn't know about character-position div
        expect(document.getElementById('someAriaLiveRegionId').innerText).toBe('the spaceship at column C, row 1 facing right on the Moon');
    });
    test('When runningState prop is changed', () => {
        const wrapper = createMountCharacterAriaLive();
        wrapper.setProps({ runningState: 'pauseRequested', world: 'Jungle' });
        // $FlowFixMe: Flow doesn't know about character-position div
        expect(document.getElementById('someAriaLiveRegionId').innerText).toBe('the jeep at column A, row 1 facing right');
        wrapper.setProps({ runningState: 'stopRequested', world: 'Space' });
        // $FlowFixMe: Flow doesn't know about character-position div
        expect(document.getElementById('someAriaLiveRegionId').innerText).toBe('the spaceship at column A, row 1 facing right on the Earth');
