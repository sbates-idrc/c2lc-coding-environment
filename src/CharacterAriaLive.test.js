...
describe('Character position gets updated on character-position div', () => {
    test('When characterState prop is changed', () => {
        ...
        expect(document.getElementById('someAriaLiveRegionId').innerText).toBe('A robot at column A, row 1 facing right');
        ...
        // $FlowFixMe: Flow doesn't know about character-position div
        expect(document.getElementById('someAriaLiveRegionId').innerText).toBe('A jeep at column B, row 1 facing right');
        ...
        // $FlowFixMe: Flow doesn't know about character-position div
        expect(document.getElementById('someAriaLiveRegionId').innerText).toBe('A rocket at column C, row 1 facing right on the Moon');
    });
    test('When runningState prop is changed', () => {
        const wrapper = createMountCharacterAriaLive();
        wrapper.setProps({ runningState: 'pauseRequested', world: 'Jungle' });
        // $FlowFixMe: Flow doesn't know about character-position div
        expect(document.getElementById('someAriaLiveRegionId').innerText).toBe('A jeep at column A, row 1 facing right');
        wrapper.setProps({ runningState: 'stopRequested', world: 'Space' });
        // $FlowFixMe: Flow doesn't know about character-position div
        expect(document.getElementById('someAriaLiveRegionId').innerText).toBe('A rocket at column A, row 1 facing right on the Earth');
        wrapper.setProps({ runningState: 'running', world: 'Sketchpad' });
        // $FlowFixMe: Flow doesn't know about character-position div
        expect(document.getElementById('someAriaLiveRegionId').innerText).toBe('A robot is moving');
